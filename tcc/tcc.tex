...
Seguindo o \textit{framework} de gamificação em engenharia de \textit{software} proposto por \citeonline{GARCIA201721}, nas seções seguintes serão apresentadas as atividades realizadas para o desenvolvimento da dinâmica gamificação para identificação de riscos em meios ágeis. O \textit{framework} prevê as seguinte etapas para esta definição:
...
\begin{enumerate}
  ...
  \item \textbf{Meta}: Desenvolver uma dinâmica gamificada que permita às equipes ágeis compreender melhor os riscos associados aos seus projetos e aumentar a percepção coletiva sobre possíveis impactos, promovendo uma análise mais estruturada e eficiente em pelo menos uma equipe piloto, com validação por \textit{feedback} qualitativo dos participantes sobre a clareza e aplicabilidade da abordagem.
\end{enumerate}
...
\begin{longtable}{|p{3cm}|p{12cm}|}
  ...
  \textit{Planning} poker dos riscos & Dinâmica que adapta o \textit{Planning Poker}. Os participantes utilizam cartas para votar sobre a probabilidade e o impacto de cada risco. O consenso obtido nas rodadas serve para priorizar os riscos mais críticos. \\ \hline
...
Esta seção detalha o processo de análise e design da dinâmica gamificada, seguindo as diretrizes da quarta etapa do \textit{framework} de \citeonline{GARCIA201721}. Serão apresentados os elementos constituintes da dinâmica, as mecânicas que governam a interação, a economia do jogo, suas regras, a estética visual e os casos de uso para sua aplicação.

\subsection{Escolha dos componentes do jogo}
\label{sec:escolha-componentes}

A dinâmica gamificada é composta por um conjunto de elementos projetados para facilitar a identificação e análise de riscos de forma interativa e engajadora. Os componentes principais são:

\begin{itemize}
\item \textbf{Cartas de risco:} São o elemento central da dinâmica, totalizando 32 cartas. Este número foi definido a partir da taxonomia original de riscos \cite{Taxonomy}, que contém 64 riscos. A seleção das 32 cartas considerou os riscos mais citados tanto no Mapeamento Sistemático da Literatura (MSL) original do trabalho de \citeonline{garcia2023agreed} quanto na continuidade do mapeamento realizada neste estudo. A soma da frequência de citação em ambos os levantamentos resultou na ordenação e seleção dos 32 riscos mais proeminentes, contemplando 84,4\% dos riscos identificados nos mapeamentos.
\begin{itemize}
\item \textbf{Aspecto visual:} As cartas possuem ilustrações únicas, geradas com o auxílio de inteligência artificial generativa (Pixlr Image Generator - \url{https://pixlr.com/br/image-generator/}). Cada ilustração foi criada a partir de prompts adaptados, inspirados no estilo “Pokémon”, com personagens que representam o risco de forma lúdica e amigável. Por exemplo, um prompt base utilizado foi: “Create a Pokémon-style image of a character related to time control powers. The character must have a friendly expression. The character must explicitly include time-related elements, such as a clock, calendar or timeline. The image must have a solid white background.”
\item \textbf{Layout e conteúdo:} O layout de cada carta foi desenvolvido para ser claro e funcional, contendo as seguintes informações:
\begin{enumerate}
\item \textbf{Nome lúdico:} Um nome criativo associado à ilustração, remetendo ao universo Pokémon (ex: “Cronoguiro” para riscos de Cronograma), visando aumentar o engajamento e a memorização.
\item \textbf{Nome do risco formal:} O nome técnico do risco conforme a taxonomia (ex: “Cronograma”).
\item \textbf{Pergunta principal:} Uma questão central que instiga a reflexão sobre a presença e a relevância do risco no projeto (ex: “O cronograma é adequado e realista?”).
\item \textbf{Perguntas secundárias/auxiliares:} Questões complementares que ajudam a aprofundar a análise do risco, direcionando a discussão (ex: “Existem dependências externas que podem causar atrasos? O método de estimativa é baseado em dados históricos? Há margem para imprevistos ou o prazo está muito apertado?”).
\item \textbf{Sinalizador de raridade:} Um símbolo visual (losango ou estrela) que indica a “raridade” da carta, um elemento gamificado inspirado nos jogos de cartas colecionáveis.
\end{enumerate}
\item \textbf{Categorias de risco:} As cartas são categorizadas de acordo com a taxonomia de riscos de \textit{software} \cite{Taxonomy}, abrangendo as categorias principais: \textit{Product Engineering}, \textit{Development Engineering} e \textit{Product Constraint}.
\end{itemize}
...
As mecânicas são os conjuntos de regras e interações que governam o fluxo da dinâmica, desde a apresentação dos riscos até a priorização. Elas foram projetadas para serem flexíveis e colaborativas, incentivando a participação ativa e o pensamento crítico sobre os riscos do projeto.

A dinâmica é dividida em duas etapas principais, com mecânicas específicas:

\subsubsection{Etapa 1: Leitura e Mapeamento dos Riscos}
\label{sec:mecanicas-leitura-mapeamento}

Esta etapa foca na identificação e associação de riscos genéricos (das cartas) a riscos reais e específicos do projeto da equipe.
...
\begin{enumerate}
  ...
  \begin{itemize}
    ...
    \item \textbf{Deck básico (12 cartas):} Composto pelas cartas numeradas de 1 a 12, que representam aproximadamente 45\% dos riscos mais citados no Mapeamento Sistemático da Literatura (MSL) e na continuidade deste trabalho.
...
\item \textbf{Mapeamento dos riscos reais (Etapa 1.2):}
  \begin{itemize}
    ...
    \item Após todos os participantes escreverem e colarem seus \textit{post-its}, o facilitador (ou um membro auto-organizado) lê os riscos registrados.
    ...
    \item O processo de leitura e mapeamento (Etapa 1.1 e 1.2) é repetido até que todas as cartas do deck escolhido sejam lidas e avaliadas.
  \end{itemize}
\end{enumerate}
...
Após a identificação e o mapeamento dos riscos, a dinâmica avança para a fase de priorização, utilizando uma mecânica inspirada no “\textit{Buy a Feature}”.

\begin{enumerate}
\item \textbf{Distribuição de moedas:} Cada participante recebe um número inicial de 3 moedas para a votação. Para incentivar a contribuição na etapa anterior, cada 5 \textit{post-its} que o participante escreveu, ele ganha uma moeda adicional, recompensando a identificação proativa de riscos.
...
O facilitador (\textit{Scrum Master}, \textit{Tech Lead}, ou outro membro com habilidades de facilitação) desempenha um papel crucial para o sucesso da dinâmica. Suas responsabilidades incluem:
\begin{itemize}
\item Preparação: Escolha e organização do deck de cartas e dos materiais.
\item Condução das etapas: Leitura das cartas, gerenciamento do fluxo das discussões, distribuição e contagem das moedas.
\item Mediação: Garantir que a discussão permaneça focada na identificação e análise de riscos, intervindo quando necessário para evitar desvios para soluções ou mitigações excessivas.
\item Gestão do tempo: Embora a dinâmica seja flexível, o facilitador deve monitorar o tempo para garantir que todas as etapas sejam concluídas de forma produtiva.
\end{itemize}
...
\subsection{Estabelecer a economia do jogo}
\label{sec:economia-jogo}

A economia dona dinâmica é um sistema simples e direto, projetado para incentivar a participação ativa dos jogadores e guiar a etapa de priorização dos riscos. Ela se baseia em um único tipo de recurso: as moedas de votação.

\subsubsection{Recursos e Ganhos}
O principal e único recurso manipulável pelos jogadores são as moedas. A distribuição de moedas segue um modelo fixo e meritocrático:
\begin{itemize}
\item \textbf{Distribuição inicial fixa:} No início da Etapa 2 (Votação), cada participante recebe uma quantidade fixa de 3 moedas. Este valor inicial garante que todos os jogadores tenham uma base para participar da priorização, independentemente de sua participação na etapa anterior.
...
\end{itemize}
Não há outros tipos de recursos a serem ganhos ou perdidos, como pontos de experiência, reputação ou penalidades, mantendo a simplicidade da economia focada na priorização.

As moedas são um recurso de uso único e são gastas de forma definitiva na Etapa 2 (Votação). Não há outras formas de gastar moedas além do investimento nos riscos priorizados, e as moedas não retornam para o jogador após a votação.

A economia de moedas exerce um impacto significativo na estratégia dos jogadores, influenciando diretamente a dinâmica da priorização:
\begin{itemize}
...
\item \textbf{Poder de voto e priorização forçada:} Ao ter um número limitado de moedas, os jogadores são compelidos a fazer escolhas difíceis e a priorizar os riscos que consideram mais críticos. Isso fomenta discussões mais focadas e um alinhamento sobre as verdadeiras preocupações da equipe.
...
\subsection{Estabelecer as dinâmicas do jogo (regras)}
\label{sec:dinamicas-regras}

As dinâmicas são definidas por um conjunto de regras que guiam a interação entre os participantes e os componentes do jogo, garantindo um fluxo colaborativo e produtivo. Diferente de jogos com turnos estritamente sequenciais, o Tarot opera em um modelo de avaliação de cartas em conjunto, onde as atividades são coordenadas principalmente pelo facilitador.

\subsubsection{Fluxo da dinâmica e coordenação}
O jogo não se estrutura em turnos individuais, mas sim em ciclos de avaliação de cada carta de risco. O fluxo é coordenado pelo facilitador e compreende as etapas citadas na Seção \ref{sec:mecanicas-leitura-mapeamento} e \ref{sec:mecanicas-votacao}.
...
\begin{itemize}
\item \textbf{Deck completo:} Aproximadamente 90 minutos.
\item \textbf{Sub-deck básico:} Aproximadamente 30 minutos.
\item \textbf{Sub-deck \textit{Product Engineering}:} Aproximadamente 30 minutos.
\item \textbf{Sub-deck \textit{Development Environment}:} Aproximadamente 30 minutos.
\item \textbf{Sub-deck \textit{Program Constraint}:} Aproximadamente 20 minutos.
\item A Etapa 2 (Votação) possui uma duração recomendada de 5 minutos, sendo uma fase mais objetiva após a discussão e mapeamento.
\end{itemize}

\subsubsection{Regras de discussão e finalização}
\begin{itemize}
\item \textbf{Gestão de divergências:} A dinâmica é projetada para capturar a diversidade de percepções. Se ao menos uma pessoa da equipe considerar que um risco é aplicável, a carta é mantida para que os \textit{post-its} relacionados possam ser gerados. Se um risco específico no \textit{post-it} for questionado, a equipe discute, mas o objetivo é identificar e mapear o máximo de riscos relevantes, não necessariamente buscar consenso sobre cada detalhe individualmente nesse momento. A ausência total de \textit{post-its} para uma carta após a discussão indica que o risco não é percebido pela equipe e a carta deve ser descartada.
\item \textbf{Finalização da dinâmica:} A dinâmica é considerada finalizada quando todas as cartas do deck selecionado foram lidas, avaliadas e os riscos mapeados (Etapa 1.1 e 1.2), e a etapa de votação e priorização (Etapa 2) foi concluída.
\item \textbf{Critério de sucesso:} Não há um critério formal de “vitória” ou “derrota” para a equipe. O sucesso da dinâmica é medido pela capacidade da equipe em gerar uma lista abrangente e priorizada de riscos relevantes ao seu projeto, promover a discussão colaborativa e aumentar a percepção coletiva sobre os desafios.
\end{itemize}

\subsubsection{Regras de conduta}
Embora não haja regras explícitas de conduta formalizadas no manual da dinâmica, pressupõe-se um ambiente de respeito mútuo, escuta ativa e engajamento. A natureza colaborativa da dinâmica depende da participação efetiva e da abertura para a discussão por parte de todos os envolvidos.
...
\item \textbf{Resultados esperados:} Uma lista clara e priorizada dos riscos mais relevantes para o projeto, um maior alinhamento da equipe sobre os desafios e um aprendizado coletivo sobre o panorama de riscos.
\end{itemize}
...
Dessa forma, a presente seção se refere ao capítulo \ref{cap:avaliacao} para aprofundamento sobre como a etapa de gerenciamento, monitoramento e medição do \textit{framework} foi operacionalizada e analisada no contexto da avaliação da dinâmica gamificada.

%------------------------------------------------------------------------------%
%------------------------------------------------------------------------------%

\chapter{Dinâmica: Guia de execução}
\label{ch:dinamica-execucao}

Este capítulo tem como objetivo apresentar uma visão geral prática da dinâmica “Tarot dos Riscos”, servindo como um guia conciso para sua preparação e execução. Serão detalhados os objetivos da dinâmica, seus componentes essenciais (com foco nos materiais visuais), e o fluxo das etapas de aplicação. Para uma compreensão aprofundada sobre a concepção e o design das mecânicas do jogo, recomenda-se consultar o Capítulo \ref{cap:proposta}. Os materiais completos da dinâmica, incluindo o manual de regras detalhado e as cartas, estão disponíveis para download conforme indicado na Conclusão.

\section{Objetivos da dinâmica}
\label{sec:objetivos-dinamica-cap5}

A dinâmica “Tarot dos Riscos” foi concebida para atender a objetivos específicos relacionados à gestão de riscos em equipes ágeis. Em suma, seus principais objetivos são:

\begin{itemize}
\item \textbf{Facilitar a identificação de riscos:} Proporcionar um método interativo e acessível para que as equipes detectem riscos potenciais em seus projetos de \textit{software}.
\item \textbf{Promover a análise colaborativa:} Estimular a discussão e o alinhamento entre os membros da equipe sobre a aplicabilidade, probabilidade e impacto dos riscos.
...
\begin{itemize}
\item \textbf{Cartas de risco:} São o elemento central da dinâmica, representando riscos potenciais em projetos de \textit{software}. Cada carta foi projetada com uma estética lúdica, inspirada no universo Pokémon, e contém informações essenciais para a discussão. A Figura \ref{anatomia-carta} ilustra a estrutura de uma carta, e a Figura \ref{carta-exemplo-tarot} apresenta um exemplo concreto.
\end{itemize}
...
A dinâmica foi concebida para ser predominantemente presencial, utilizando materiais físicos. No entanto, para ampliar sua aplicabilidade, foi desenvolvida uma adaptação para o ambiente online, sem a necessidade de uma plataforma de \textit{software} complexa, conforme detalhado na Seção \ref{sec:desenvolvimento-plataforma-gamificada} do Capítulo \ref{cap:proposta}.

\begin{itemize}
\item \textbf{Formato presencial:} Requer a impressão das cartas de risco, das moedas e a utilização de \textit{post-its}. A dinâmica é conduzida com os participantes reunidos em torno de uma mesa, onde as cartas são dispostas e os \textit{post-its} colados.
...
\begin{enumerate}
\item \textbf{Escolher o deck de cartas:} Selecionar o deck mais adequado às necessidades da equipe (Deck Completo, Deck Básico ou Sub-decks temáticos), considerando o tempo disponível e o foco da sessão.
\item \textbf{Organizar os materiais:} Preparar as cartas de risco (impressas ou no quadro online), as moedas (físicas ou representações digitais) e garantir a disponibilidade de \textit{post-its} e canetas.
...
\begin{enumerate}
\item \textbf{Leitura e avaliação das cartas:} O facilitador (ou membro auto-organizado) lê uma carta de risco por vez. A equipe discute se o risco é relevante para o projeto. Se houver relevância percebida por ao menos um membro, a carta é mantida na mesa. Caso contrário, é descartada.
\begin{figure}[H]
  \centering
...
Será detalhado o planejamento da avaliação, incluindo a metodologia empregada e as adaptações realizadas no método MEEGA+. Em seguida, descreve-se a execução da avaliação, dividida em duas abordagens distintas: uma com a participação de estudantes e outra com profissionais da área de desenvolvimento de \textit{software}. Para cada uma dessas avaliações, serão apresentados os métodos de coleta e análise dos dados. Por fim, o capítulo traz uma discussão aprofundada dos resultados obtidos, comparando as percepções dos grupos, e finaliza com a discussão das ameaças à validade que podem influenciar as conclusões deste estudo.

\section{Planejamento da avaliação}

Para garantir a validade e a robustez dos resultados da dinâmica proposta, foi empregado uso do modelo MEEGA+ (Method for an Extensive and Exhaustive Game Assessment) \cite{MEEGA}, um \textit{framework} consolidado para a avaliação da qualidade de jogos e experiências gamificadas.
...
\begin{itemize}
  ...
  \item Estruturação da análise: Houve sugestões para padronizar a forma de registrar os riscos (um \textit{post-it} por risco) e, em alguns casos, para uma organização das cartas (ex.: diagrama de Ishikawa) que pudesse guiar a identificação de riscos de forma mais estruturada.
...
\begin{itemize}
  ...
  \item Repetitividade e sobreposição de cartas: Similar ao \textit{feedback} dos estudantes, houve menção a algumas cartas que pareciam ter temas muito parecidos ou elementos difíceis de diferenciar (ex:  “Stakeon” e  “Clientor”). A sugestão é revisar os textos ou o número de cartas para evitar redundâncias.
